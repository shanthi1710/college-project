...
const RegisterUser = () => {
  ...
  const initialValues = {
    userid: parseInt(unRegisteredGoogleUser?.displayName.substring(0,4)), // 8th character is the space which is excluded
    username: unRegisteredGoogleUser?.displayName.substring(5), // the charcters starting from 9th index to the end of string is taken
    email: unRegisteredGoogleUser?.email,
    passoutyear:
      parseInt(
        unRegisteredGoogleUser?.email.substring(
          unRegisteredGoogleUser?.email.length - 10,
          unRegisteredGoogleUser?.email.length - 12
        )
      ) + 2004,
    avatar: [],
  };
